---
title: "(Ineffective) Messages to Encourage Recycling: Evidence from a Randomized Evaluation in Peru"
rct_id: "AEARCTR-0000139"
rct_id_num: "139"
doi: "10.1257/rct.139-1.0"
date: "2016-05-18"
status: "completed"
jel: "D03, Q53, N56, C93"
start_year: "2010-01-08"
end_year: "2013-05-01"
pi: "Dean Karlan"
pi_other:
  1:
    name: Alberto Chong
    email: achong@uottawa.ca
    affiliation: School of International Development and Global Studies and  Department of Economics, University of Ottawa
  2:
    name: Jeremy Shapiro
    email: jeremypshapiro@gmail.com
    affiliation: GiveDirectly (current)
  3:
    name: Yale University
    email: former
    affiliation: 
  4:
    name: Jonathan Zinman
    email: jzinman@dartmouth.edu
    affiliation: Dartmouth College and Innovations for Poverty Action
abstract: "There is growing interest in using messaging to drive pro-social behaviors, which contribute to investment in public goods. We worked with a leading NGO in Peru to randomize nine different pro-recycling messages that were crafted based on best practice, prior evidence, and theories of behavioral change. Different variants emphasized information on environmental or social benefits, social comparisons, social sanctions, authority, and/or reminders. None of the messages had significant effects on recycling behavior. However, reducing the cost of ongoing participation--by providing a recycling bin--significantly increased recycling among enrolled households. "
layout: registration
---

